public class Main{
	...
	public static int[][] sort(String a){
		...
		for(int i = 0; i < a.length(); i++){
		    if(a.charAt(i) == ','){
		        c[id][vote] = Integer.parseInt(a.substring(last, i));
		        vote++;
		        last = i + 1;
		    }
		    if(a.charAt(i) == ';'){
	...
	public static int[][] remove(int[][] c){
	    int check = 0;
		while(check < c.length){
		    for(int i = 0; i < c.length; i++){
		        if(c[check][0] == c[i][0] && i != check){
		            c[i][1] = 0;
		            c[i][2] = 0;
		            c[i][3] = 0;
		            c[i][4] = 0;
	...
	public static int[] tally(int[][] a, int c){
	    int[] b = new int[4];
	    for(int i = 0; i < a.length; i++){
		    for(int j = 0; j < 5; j++){
		        if(j == c){
	...
	public static void result(int a[], int b, String[] c){
	    for(int i = 0; i < a.length; i++){
            if(b == 0)
                System.out.println("Votes for "+c[i]+": "+a[i]);
            if(b == 1)
                System.out.println("Votes for "+c[i]+": "+a[i]);
            if(b == 2)
                System.out.println("Votes for "+c[i]+": "+a[i]);
		...
		System.out.printf("%s: "+c[win]+"\n\n", (b == 0) ? "President" : (b == 1) ? "Vice President" : (b == 2) ? "Secretary" : "Treasurer");
	}
}
